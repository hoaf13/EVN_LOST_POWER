...
def pred_intent(previous2_action, previous1_action, message, model):
    ...
    # intent_X == action_X
    if previous2_action == previous1_action or label[6:] == previous1_action[6:]:
...
def generate_text(previous_action, action, provided_entities):
    ...
    if action == 'action_start' and previous_action == action:
        ...
        text = 'Xin chào quý khách {}, đây là tổng đài tự động của EVN. Xin hỏi anh chị báo mất điện ở khu vực quận, huyện nào vậy.'.format(name)
        ...
        text = "Hệ thống ghi nhận mất điện tại quận/huyện {}. Anh chị vui lòng cho em biết là, tình trạng mất điện chỉ ở nhà anh chị hay trên cả khu vực luôn vậy.".format(district_name)
        ...
        district_name = provided_entities['value'] 
        text = "Hệ thống ghi nhận mất điện trên cả khu vực quận/huyện {}. Anh chị vui lòng chờ trong giây lát để em kiểm tra lịch cắt điện tại khu vực của mình.".format(district_name)
...
def save_to_db(sender_id, action, value, client_message, bot_message):
    conversation = Conversation(sender_id=sender_id, action=action, value=value, client_message=client_message, bot_message=bot_message)
